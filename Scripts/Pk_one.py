...
class Pk:
    def __init__(self,delta,L,Nside,l,k,grid_info,binning_info,
        ...
        def sum_loop(F_1,F_2,composite=False):
            ...
            power_k = F_1*MAS*F_2*MAS#is conjugate as F(-k) = F*(k)
            ########################################################################## edit
        ...
        def Pk_endpoint(delta,k,l):
            ...
            ############################################################### edit
            power_k = delta_l*delta_kk#has to be conjugate as this creates the other part from from rfft - so if you consider full fft then we get double N_modes and Power_bin which cancels
